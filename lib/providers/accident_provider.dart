...
class AccidentProvider extends ChangeNotifier {
  ...
  Future<bool> rejectCurrentAccident({int? driverId}) async {
    if (_currentAccident == null) return false;

    final effectiveDriverId = driverId ?? _lastDriverId;
    if (effectiveDriverId == null) {
      print('Reject accident error: Driver ID not available');
      _setError('Driver information missing. Please reload accidents and try again.');
      return false;
    }
    ...
    try {
      ...
      print('Driver ID: $effectiveDriverId');

      final accidentId = _currentAccident!.id;

      // Call API to record rejection
      final apiResult = await CentralizedApiService.rejectAccident(
        driverId: effectiveDriverId,
        accidentId: accidentId,
      );

      if (!apiResult) {
        print('Reject accident API returned false for accident $accidentId');
        _setError('Failed to reject accident report. Please try again.');
        return false;
      }
      ...
      // Remove accident locally
      _allAccidents.removeWhere((accident) => accident.id == accidentId);
      _accidentList.removeWhere((accident) => accident.id == accidentId);
      
      // Recalculate current accident and pending count
      _applyFilter();
      notifyListeners();
