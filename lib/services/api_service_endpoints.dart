...
class CentralizedApiService {
  ...
  /// Reject accident API
  static Future<bool> rejectAccident({
    required int driverId,
    required int accidentId,
  }) async {
    try {
      final url = ApiEndpoints.getAccidentUrl(ApiEndpoints.getAccidents);
      final payload = {
        'action': 'reject_accident',
        'driver_id': driverId,
        'accident_id': accidentId,
      };

      print('🔴 Reject Accident - URL: $url');
      print('🔴 Reject Accident - Payload: $payload');
      ...
      print('🔴 Reject Accident - Status: ${response.statusCode}');
      print('🔴 Reject Accident - Body: ${response.body}');

      if (response.statusCode != 200) {
        throw Exception('Reject accident failed: ${response.statusCode}');
      }
      ...
      if (data is Map<String, dynamic>) {
        ...
        final message = data['message'] ?? data['error'] ?? 'Failed to reject accident';
        throw Exception(message);
      } else {
        throw Exception('Invalid response from server');
      }
    } catch (e) {
      print('Error rejecting accident: $e');
      rethrow;
    }
  }
