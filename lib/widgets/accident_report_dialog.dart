...
class _AccidentReportDialogState extends State<AccidentReportDialog> {
  ...
  void _callClient(AccidentReport accident) async {
    try {
      ...
      final response = await http.get(
        Uri.parse('https://tosssolution.com/apatkal/api/get_client_mobile.php?accident_id=${accident.id}'),
      );
  ...
  void _handleReject(AccidentProvider accidentProvider) async {
    // Get driver details from AuthProvider
    final authProvider = Provider.of<AuthProvider>(context, listen: false);
    final driverId = int.tryParse(authProvider.currentUser?.driverId ?? '') ?? 0;

    if (driverId == 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Driver information missing. Please log in again.'),
          backgroundColor: Colors.red,
        ),
      );
      return;
