...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($input['action'])) {
            if ($input['action'] === 'accept_accident') {
                ...
                exit;
            } elseif ($input['action'] === 'reject_accident') {
                // Reject accident and record in rejected_reports
                $accident_id = intval($input['accident_id'] ?? 0);
                $driver_id = intval($input['driver_id'] ?? 0);
                
                if ($accident_id <= 0 || $driver_id <= 0) {
                    sendErrorResponse('accident_id and driver_id are required for rejection');
                }
                
                // Get driver info
                $driverStmt = $pdo->prepare("SELECT id, driver_name FROM drivers WHERE id = ?");
                $driverStmt->execute([$driver_id]);
                $driver = $driverStmt->fetch(PDO::FETCH_ASSOC);
                if (!$driver) {
                ...
                $accident = $accidentStmt->fetch(PDO::FETCH_ASSOC);
                if (!$accident) {
                    sendErrorResponse('Accident not found');
                }
                
                // Resolve client_id by matching accident phone with clients.mobile_no (if exists)
                $clientId = null;
                try {
                    $clientStmt = $pdo->prepare("SELECT id FROM clients WHERE mobile_no = ? LIMIT 1");
                    $clientStmt->execute([$accident['phone']]);
                    $clientId = $clientStmt->fetchColumn();
                } catch (Exception $e) {
                    // ignore if clients table not present
                }
                
                // Ensure rejected_reports table exists (create if not)
                try {
                    $pdo->exec("
                    ...
                    error_log("rejected_reports ensure table failed: " . $e->getMessage());
                }
                
                // Insert into rejected_reports (idempotent per driver+accident)
                $insertStmt = $pdo->prepare("
                    INSERT INTO rejected_reports
                        (driver_id, driver_name, client_id, accident_id, fullname, phone, vehicle, accident_date, location, latitude, longitude, description, created_at)
                    VALUES
                        (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ");
                ...
                if ($ok) {
                    echo json_encode(['success' => true, 'message' => 'Accident rejected and recorded']);
                } else {
                    echo json_encode(['success' => false, 'message' => 'Failed to record rejection']);
                }
                exit;
            }
        }
    ...
    // Preload rejected accidents for this driver to exclude them from listings
    $rejectedAccidentIds = [];
    if ($driver_id > 0) {
        try {
            $rejCheck = $pdo->prepare("SHOW TABLES LIKE 'rejected_reports'");
            $rejCheck->execute();
            if ($rejCheck->fetch()) {
                $rejIdsStmt = $pdo->prepare("SELECT accident_id FROM rejected_reports WHERE driver_id = ?");
                $rejIdsStmt->execute([$driver_id]);
                $rejectedAccidentIds = array_map('intval', $rejIdsStmt->fetchAll(PDO::FETCH_COLUMN) ?: []);
            }
        } catch (Exception $e) {
            error_log("Rejected reports exclusion failed: " . $e->getMessage());
        }
    }
    ...
    if ($driver_location) {
        ...
        foreach ($all_accidents as $accident) {
            // Exclude accidents this driver has rejected
            if (!empty($rejectedAccidentIds) && in_array((int)$accident['id'], $rejectedAccidentIds, true)) {
                continue;
            }
