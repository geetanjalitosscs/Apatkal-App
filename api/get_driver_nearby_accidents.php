...
try {
    // Preload rejected accidents for this driver to exclude them
    $rejectedAccidentIds = [];
    try {
        $rejStmt = $pdo->prepare("SHOW TABLES LIKE 'rejected_reports'");
        $rejStmt->execute();
        if ($rejStmt->fetch()) {
        ...
        // If table missing or error, proceed without exclusion
        error_log("Rejected reports preload failed: " . $e->getMessage());
    }
    ...
    foreach ($all_accidents as $accident) {
        // Exclude accidents rejected by this driver
