...
$api_config = [
    'base_url' => 'https://tosssolution.com/apatkal/api/',
    'uploads_url' => 'https://tosssolution.com/apatkal/api/uploads/',
    'uploads_base_url' => 'https://tosssolution.com/apatkal/uploads/',
    'site_url' => 'https://tosssolution.com/apatkal/'
];
...
/**
 * Ensure PHP session is started using a writable path.
 *
 * @param string|null $sessionId Optional session ID to resume.
 * @return void
 */
function ensureSessionStarted($sessionId = null) {
    static $sessionInitialised = false;
    ...
    } elseif ($sessionId && session_status() !== PHP_SESSION_ACTIVE) {
        session_id($sessionId);
    }
    ...
    if (!$sessionInitialised) {
        ...
        if (!is_dir($sessionPath)) {
            if (!mkdir($sessionPath, 0770, true) && !is_dir($sessionPath)) {
                error_log("ensureSessionStarted: Failed to create session directory at {$sessionPath}");
            }
        }

        if (is_dir($sessionPath) && !is_writable($sessionPath)) {
            error_log("ensureSessionStarted: Session directory {$sessionPath} is not writable");
        }
    ...
    if (session_status() !== PHP_SESSION_ACTIVE) {
        if (!@session_start()) {
            error_log('ensureSessionStarted: session_start failed using path ' . session_save_path());
        }
    }
}
