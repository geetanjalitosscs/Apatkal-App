...
- Centralized `ensureSessionStarted()` helper stores PHP sessions in `api/storage/sessions` so OTP data persists reliably across requests
...
2. Import database/edueyeco_apatkal2.sql (or the latest tosssytd_apatkal_new.sql) into MySQL
...
```dart
static const String baseUrl = 'https://tosssolution.com/apatkal/api/';
```
...
- Edit `api/db_config.php`:
  ...
  $db_config = [
      'host' => 'localhost',
      'dbname' => 'tosssytd_apatkal',
      'username' => 'tosssytd_apatkal',
      'password' => 'tosssytd_apatkal',
  ];
  ```
- Create the session storage directory (or ensure it remains writable) so that OTP flows can persist data:
  ```bash
  mkdir -p api/storage/sessions
  chmod 770 api/storage/sessions
  ```
  The new helper `ensureSessionStarted()` automatically points PHP sessions to this path and reuses incoming `PHPSESSID` cookies.
...
- Confirm the session directory `api/storage/sessions` exists and is writable; otherwise PHP cannot persist OTP data and the API returns "OTP session expired."
...
- ✅ **Centralized Session Handling** - `ensureSessionStarted()` redirects PHP sessions to `api/storage/sessions`, fixing OTP expiry issues on shared hosting
